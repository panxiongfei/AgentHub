...
class ManusPlatform(BasePlatform):
    """Manus平台实现"""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Manus相关配置
        self.base_url = config.get("base_url", "https://manus.ai")
    ...
    async def _connect_to_existing_browser(self) -> None:
        ...
        try:
            ...
            if not contexts:
                raise PlatformConnectionError("manus", Exception("没有找到浏览器上下文"))
            ...
            for page in pages:
                url = page.url
                if "manus" in url.lower() or "manus.ai" in url.lower():
                    manus_page = page
                    break
            
            if manus_page:
                self.page = manus_page
                self.logger.info(f"找到Manus页面: {self.page.url}")
            else:
                # 如果没有找到Manus页面，创建新标签页
                self.page = await self.context.new_page()
                await self.page.goto(self.base_url)
                self.logger.info("创建新的Manus页面")
            
        except Exception as e:
            self.logger.error(f"连接浏览器失败: {e}")
            raise PlatformConnectionError("manus", e)
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if self.page:
                ...
                if "manus" in title.lower() or "登录" in title or "dashboard" in title.lower():
                    self.logger.info("Manus平台连接成功")
                    return True
                else:
    ...
    async def submit_task(
        ...
        **kwargs
        ...
        try:
            ...
            if not input_element:
                # 如果还是找不到，截图保存用于调试
                screenshot_path = Path("data/temp/manus_debug.png")
                screenshot_path.parent.mkdir(parents=True, exist_ok=True)
                await self.page.screenshot(path=str(screenshot_path))
                
                raise PlatformError(
                    "无法找到输入框，请检查Manus页面是否正确加载",
                    platform="manus"
                )
            ...
            # 生成任务ID（时间戳）
            task_id = f"manus_{int(time.time())}"
            ...
            raise PlatformError(f"提交任务失败: {e}", platform="manus")
            ...
            for selector in error_selectors:
                try:
                    ...
                    if error_element and await error_element.is_visible():
                        ...
                        if error_text and ("积分" in error_text or "insufficient" in error_text.lower() or "error" in error_text.lower()):
    ...
    async def get_task_result(self, task_id: str) -> TaskResult:
        ...
        try:
            ...
            # 截断过长的结果用于日志显示
            display_result = result_text[:1000] + "..." if len(result_text) > 1000 else result_text
    ...
    async def download_files(self, task_id: str, download_dir: Path) -> List[Path]:
        ...
        try:
                    ...
                    for element in elements:
                        if await element.is_visible():
                            try:
                                ...
                                # 保存文件
                                filename = download.suggested_filename or f"manus_file_{len(downloaded_files)}.bin"
            ...
            try:
                # 1. 保存页面截图（全页面）
                screenshot_path = download_dir / f"manus_screenshot_{task_id}.png"
            ...
            try:
                # 2. 保存页面HTML
                html_path = download_dir / f"manus_page_{task_id}.html"
            ...
            try:
                ...
                if result.result and len(result.result.strip()) > 10:
                    text_path = download_dir / f"manus_result_{task_id}.txt"
            ...
            try:
                # 4. 保存元数据
                metadata_path = download_dir / f"manus_metadata_{task_id}.json"
                import json
                metadata = {
                    "task_id": task_id,
            ...
            # 即使出错也尝试保存基本信息
            try:
                error_log_path = download_dir / f"manus_error_{task_id}.txt"
    ...
    async def execute_full_task(
        ...
        **kwargs
        ...
        try:
            if not task_id:
                task_id = f"manus_interrupted_{int(time.time())}"
            ...
            result = TaskResult(
                platform="manus",
                task_id=task_id or f"manus_error_{int(time.time())}",
                success=False,
                result=f"获取结果失败: {e}",
